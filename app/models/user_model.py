...
class User:
    ...
    def create_user(cls, user):
        ...
        params = (user.username, generate_password_hash(user.password))
    ...
    def get_users(cls, server_id: int) -> list["User"]:
        ...
        users_list = []
        for user in users:
            user_data = User(user_id=user[0], username=user[1], image=user[2])
    ...
    def get_user(cls, user_id: int):
        ...
        if user_data is not None:
            return User(user_id=user_data[0], username=user_data[1], image=user_data[2])
        else:
            return None
